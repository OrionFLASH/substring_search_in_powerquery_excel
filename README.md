...
Скрипт `src/generate_gsz_keys.py` читает локальный `КейЛОАД.txt` и создаёт Excel локально.
...
|----------|--------------|----------|
| `input_file` | `КейЛОАД.txt` | Локальный входной список (не хранится в git) |
...
| 1.11 | 2026-07-13 | Плейсхолдер нескольких совпадений: `=>` → `:=>` |
