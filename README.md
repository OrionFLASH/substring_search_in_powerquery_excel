...
Ускоренные версии не меняют формулу совпадения, но сначала отсекают кандидатов по **всем** непустым AND/OR-токенам (`full` и `not`) — ключи не отбрасываются до одного «якоря».

| Состав ключей строки | Что индексируется | Кандидат, если… |
|----------------------|-------------------|-----------------|
| Любые `and_*` / `or_*` | **Все** непустые full/not токены | найден **хотя бы один** |
| Только `non` | без положительных якорей | строка всегда кандидат (далее полная проверка + исключения) |

Полная проверка по-прежнему: AND (все + без пересечений) → OR (хотя бы один) → `and_non` / `or_non`.  
Так не теряются пары латиница/кириллица и любые сочетания ключей.  
...
2. Предфильтр кандидатов: **все** AND/OR-токены (`full`/`not`) — достаточно любого совпадения; ключи не отсекаются.
3. Полная проверка `fnСтрокаGszСовпала` запускается только по кандидатам, прошедшим предфильтр.
...
Дополнительно:
- использует предфильтр по **всем** AND/OR-токенам (`full`/`not`; для `not` — через `positions_not`);
...
Списки `and_*_cols` / `or_*_cols` задают колонки справочника: в Python и в FAST/FAST_V2 **все** непустые full/not-токены строки участвуют в предфильтре (достаточно любого совпадения). Отдельный флаг в config не нужен — это обязательное поведение. Группы `*_non` и `key_fix_id` работают как раньше на этапе полной проверки.
...
Проверка, что предфильтр (все AND/OR-токены) даёт тот же результат, что и полный перебор:
...
|---------|------------|--------|
| `ИНН` | Уникальный номер | 30 |
| `Наименование` | Названия из «Компании группы (ЕРЗ)»; разные через `";\n"` | 70 |
| `Наименование, регион` | Значения с исходных строк; разные через `";\n"` | 100 |
| `Кол-во регионов` | Число уникальных «Наименование, регион» | 10 (центр) |
| `Наименование без города` | Уникальные части до первой запятой (`ПИК, Москва` → `ПИК`) | 90 |
| `Кол-во наименований без города` | Число уникальных урезанных названий | 10 (центр) |

### Форматирование (Python)

- заголовки: жирный, центр по горизонтали/вертикали, перенос слов;
- автофильтр; закрепление 1-й строки и **1-й колонки** (`B2`);
- данные: вертикальный центр, выравнивание влево (колонки количеств — по центру);
- ширины колонок — см. таблицу выше / `output_format.columns` в config.
...
3. В модалке:
   - настроить паузу, **источник компаний** (по умолчанию **только names**), names/join one|all,
     режим **пагинации** (on/off), **пачку групп** (batch, по умолчанию **1000**) и
     **чекпоинт каждые N** (по умолчанию **250**);
   ...
   - при включённой пагинации кнопка «Запросить компании» теперь автоматически запускает следующую пачку до полного завершения pending-групп.

`costType=1` всегда. У группы в JSON: `status` (`pending`/`done`/`error`/`partial`), `errors[]`.

### Статистика в модалке (live, не лог)

В блоке `erz-stats` во время выполнения показывается:
- сколько групп загружено raw и сколько осталось после дедупликации;
- текущая обрабатываемая группа, alias текущего запроса (`brand/join`, `developer/join`, `developer/names`);
- прогресс по выбранным группам;
- для текущей группы: число уникальных компаний по `names` (id);
- тайминги:
  - время последнего запроса;
  - среднее по текущей группе;
  - среднее за всё время;
  - среднее за последние 10 и 100 запросов;
- тренд `avg10/avg100` относительно `avg all`:
  - `🔺` — замедление (среднее выросло),
  - `🟢🔻` — ускорение (среднее снизилось),
...
### Дедуп-метрики в логе

- При сборе групп (JOIN по регионам): `raw -> unique` динамически после каждого региона.
- При `developer/join` и `developer/names`: для каждого запроса выводится:
  `+сырой N · дедуп A -> B`.
- Для групп: `Группа X/Y (из Z)`, где `Z` — общее число выбранных групп.

### Оптимизации длинного прогона

Чтобы убрать деградацию после тысяч запросов:
- ограничен размер DOM-лога (старые строки удаляются);
- обновление статистики троттлится через `requestAnimationFrame`;
- ререндер списка групп выполняется реже (пакетно).
...
| 1.23 | 2026-07-29 | ERZ JS: опциональная пагинация (1000/250), расширенная live-статистика, дедуп-метрики JOIN/NAMES, оптимизации UI |
| 1.24 | 2026-07-29 | ERZ JS: при конце пачки автоматически запускается следующая пачка до завершения pending |